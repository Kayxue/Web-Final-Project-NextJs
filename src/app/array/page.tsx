...
export default function aboutus() {
    ...
    async function getStackFromWeb() {
        try {
            const response: AxiosResponse | Error = await axios.get("https://web-final-project-example-api.vercel.app/array").then(r => r).catch(e => e)
    ...
    return (
        <div>
            ...
            <div className="button_group" style={{ float: "left" }}>
                <div id="assign" className="button" onClick={() => openAssignSizeInput()}>Size</div><br />
                <div id="assignValue" className="button" onClick={() => openAssignValueInput()}>Input</div><br />
                <div id="delete" className="button" onClick={() => openDeleteInput()}>Delete</div>
            </div>

            <div className="board" id="board">
                {
                    boardExist ?
                        <Board array={array} showNow={showNow} />
                        : null
                }
            </div>

            <div id="input">
                {assignSizeInputHidden ? null : <AssignSizeInput arraySize={arraySizeStr} setArraySize={setArraySizeStr} setAssignInputHidden={setAssignSizeInputHidden} assignSize={assignSize} />}
                {pushAssignInputHidden ? null : <AssignValueInput pushValueStr={pushValueStr} setPushValueStr={setPushValueStr} pushIndexStr={pushIndexStr} setPushIndexStr={setPushIndexStr} setPushAssignInputHidden={setPushAssignInputHidden} assignValue={assignValue} />}
                {deleteInputHidden ? null : <DeleteInput deleteIndex={deleteIndexStr} setDeleteIndex={setDeleteIndexStr} setDeleteInputHidden={setDeleteInputHidden} deleteVal={deleteValue} />}
                {searchIndexInputHidden ? null : <SearchInput searchInddex={searchIndexValueStr} setSearchIndex={setSearchIndexValueStr} setSearchInputHidden={setSearchIndexInputHidden} searchValue={searchIndex} />}
            </div>

            <div className="button_group" style={{ float: "right" }}>
                <div id="search" className="button" onClick={() => openSearchInput()}>Search</div><br />
                <div id="clear" className="button" onClick={() => clearAll()}>Clear</div><br />
                <div id="example" className="button" onClick={() => getExample()}>Example</div>
            </div>
        </div>
    )
}